...
val GAME_ID_PATTERN = Pattern.compile("(Game )([0-9]+).*")

fun main() {
    val fullDrawing = listOf(Pick("red", 12), Pick("green", 13), Pick("blue", 14))
    val games = input.split("\n")
        .map { gameString -> Game(extractGameId(gameString), extractDrawings(gameString)) }

    val result1 = games.filter { game -> game.isPossible(fullDrawing) }
        .sumOf { game -> game.id }


    val result2 = games.map(Game::minimalPicks)
        .sumOf { picks -> picks.map { pick -> pick.number }.reduce { value, acc -> acc * value } }

    println("Result 1 = $result1")
    println("Result 2 = $result2")
}

fun extractGameId(gameString: String): Int {
    return GAME_ID_PATTERN.matcher(gameString).results().findFirst().map { result -> result.group(2).toInt() }.orElse(0)
}

fun extractDrawings(gameString: String): List<List<Pick>> {
    return gameString.split(": ")[1].split(";")
        .map { drawing -> extractPicks(drawing) }
}

private fun extractPicks(drawing: String): List<Pick> {
    return drawing.split(",")
        .map { pick -> pick.trim() }
        .map { pick -> pick.split(" ") }
        .map { components -> Pick(components[1], components[0].toInt()) }
}
